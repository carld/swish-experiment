...
;;;;;; File header: %3a14/char-sets/inversion-list.sls
;; -*- mode: scheme; coding: utf-8 -*-
;; Copyright © 2018 Göran Weinholt <goran@weinholt.se>
;; SPDX-License-Identifier: (MIT OR BSD-3-Clause OR LicenseRef-LICENSE)
#!r6rs
;;;;;; File header: %3a14/char-sets/inversion-list-impl.scm
; Part of Scheme 48 1.9.  See file COPYING for notices and license.
(library (srfi :14 char-sets inversion-list)
  (export inversion-list? make-empty-inversion-list
   number->inversion-list numbers->inversion-list
   range->inversion-list ranges->inversion-list
   inversion-list=? inversion-list-adjoin
   inversion-list-complement inversion-list-copy
   inversion-list-difference inversion-list-fold/done?
   inversion-list-hash inversion-list-intersection
   inversion-list-member? inversion-list-remove
   inversion-list-size inversion-list-union
   inversion-list-cursor inversion-list-cursor?
   inversion-list-cursor-ref inversion-list-cursor-next
  ...
  (let-syntax ([define-record-type (lambda (x)
                                     (syntax-case x ()
                                       [(_ tag typename
    ...
    (begin
      ...
      (define-record-discloser
        ...
        (lambda (r)
          (list
            'inversion-list
            (inversion-list-min r)
            (inversion-list-max r)
      ...
      (define (inversion-list-member? n i-list)
        (let ([ranges (inversion-list-range-vector i-list)])
          (let loop ([low 0] [high (vector-length ranges)])
            (if (< low high)
            ...
            (cond
              ...
              [(not (= min (vector-ref ranges 0)))
               (if (and (even? size)
                   ...
                   (let ([result (make-vector (+ 1 size))])
                     ...
                     result))]
              [(and (even? size) (= max (vector-ref ranges (- size 1))))
              ...
              [else
               (let ([result (make-vector (- size 1))])
                 ...
                 result)]))))
      (define (make-inversion-list-union/intersection proc-thunk
               write-increment-count write-decrement-count process-first?
               decrement-count? middle-increment copy-extra-count)
            ...
            (let ([size-1 (vector-length ranges-1)]
              ...
              (let ([temp (make-vector (+ size-1 size-2))])
                (let loop ([index-1 0]
                           ...
                           [count 0]
                           [index-result 0])
                  (if (and (< index-1 size-1) (< index-2 size-2))
                            ...
                            (if (even? index)
                                ...
                                (if (= write-decrement-count count)
                                    ...
                                    (loop
                                      ...
                                      (- count 1)
                                      index-result))))))
                      (let* ([count (if (or (and (not (= index-1 size-1))
                                                 (decrement-count?
                                                   index-1))
                                        ...
                                        count)]
                             [result-size (if (= copy-extra-count count)
                                              (+ index-result
                                                 (- size-1 index-1)
                                                 (- size-2 index-2))
                                              index-result)]
                        ...
                        (if (= copy-extra-count count)
                            (begin
                              (vector-copy! ranges-1 index-1 result
                                index-result (- size-1 index-1))
                              (vector-copy! ranges-2 index-2 result
                                index-result (- size-2 index-2))))
      ...
      (define inversion-list-union
        (binary->n-ary
          (make-inversion-list-union/intersection (lambda () inversion-list-union) 0 1 even? odd? -1 0)))
      (define inversion-list-intersection
        (binary->n-ary
          (make-inversion-list-union/intersection (lambda () inversion-list-intersection) 1 2 odd? even? 1
            2)))
      ...
      (define (number->inversion-list min max n)
        (if (or (< n min) (>= n max))
            (assertion-violation 'number->inversion-list
              "invalid number"
              min
              max
              n))
        (make-inversion-list
          min
          max
          (if (= n (- max 1)) (vector n) (vector n (+ n 1)))))
      ...
      (define (range->inversion-list min max left right)
        (if (or (> min max)
                ...
                (< left min)
                (> right max))
            (assertion-violation 'range->inversion-list
              "invalid range"
              min
              max
              left
        ...
        (make-inversion-list
          min
          max
          (if (= right max) (vector left) (vector left right))))
        ...
        (let loop ([ranges ranges]
          ...
          (if (null? ranges)
              ...
              (let ([range-pair (car ranges)])
                (let ([left (car range-pair)] [right (cdr range-pair)])
                  (if (not (and (number? left) (number? right)))
                      (assertion-violation 'ranges->inversion-list
                        "invalid range"
                        min
                        max
                        (cons left right)))
      ...
      (define (inversion-list=? i-list-1 i-list-2)
        (and (= (inversion-list-min i-list-1)
             ...
             (= (inversion-list-max i-list-1)
                (inversion-list-max i-list-2))
             (equal?
               (inversion-list-range-vector i-list-1)
        ...
        (let ([index (inversion-list-cursor-index cursor)]
          ...
          (let* ([ranges (inversion-list-range-vector i-list)]
            ...
            (cond
              ...
              [(< (+ index 2) size)
               (make-inversion-list-cursor
                 (+ index 2)
                 (vector-ref ranges (+ index 2)))]
              [else (make-inversion-list-cursor #f #f)]))))
            ...
            (let loop ([i 0] [ans 0])
              (if (>= i size)
                  ...
                  (loop
                    ...
                    (bitwise-and
                      mask
                      (+ (* 37 ans) (vector-ref range-vector i)))))))))
      (define (vector-copy! source source-start dest dest-start
               count)
        (let loop ([i 0])
          (if (< i count)
              (begin
      ...
      (define (vector-copy v)
        (let* ([size (vector-length v)] [copy (make-vector size)])
