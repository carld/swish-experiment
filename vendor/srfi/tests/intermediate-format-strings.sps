...
;;;;;; File header: ./tests/intermediate-format-strings.sps
#!r6rs
(import
  (rnrs)
  (rnrs mutable-pairs)
...
(define-syntax expect
  (syntax-rules ()
    [(_ expected expr) (check expr => expected)]))
...
(check (format "~6F" 32.0) (=> member) '("  32.0" "   32."))
(check
  (format "~8F" 3.2e46)
  (=> member)
  '("  3.2e46" " 3.2e+46"))
(expect " 3.2e-44" (format "~8,1F" 3.2e-44))
(check
  (format "~8F" 3.2e21)
  (=> member)
  '("  3.2e21" " 3.2e+21"))
...
(check (format "~0,3F" 20263/2813) (=> member) '("7.203"))
(check (format "~0,2F" 20263/2813) (=> member) '("7.20"))
...
(check
  (format
    "~w"
    (let ([c (list 'a 'b 'c)]) (set-cdr! (cddr c) c) c))
  (=> member)
  '("#0=(a b c . #0#)" "#1=(a b c . #1#)"))
(expect
  "\n"
...
(test '("~f" -314.0) "-314.0")
(check (format "~f" 10000.0) (=> member) '("1e4" "10000.0"))
(check
  (format "~f" -1.23e10)
  (=> member)
