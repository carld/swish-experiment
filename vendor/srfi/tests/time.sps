...
;;;;;; File header: ./tests/time.sps
#!r6rs
;; Copyright 2010 Derick Eddington.  My MIT-style license is in the file named
;; LICENSE from the original collection this file is distributed with.
;;;;;; File header: %3a19/srfi-19-test-suite.scm
;;; simple test procedures
(import (rnrs) (rnrs mutable-pairs)
  (srfi :48 intermediate-format-strings)
  (srfi private include) (srfi :19 time))
(begin
  ...
  (define (define-s19-test! name thunk)
    (let ([name (if (symbol? name) name (string->symbol name))]
      ...
      (if pr
          (set-cdr! pr thunk)
          (set! s19-tests
            (append s19-tests (list (cons name thunk)))))))
  ...
  (define (run-s19-tests . verbose)
    (let ([runs 0]
          [goods 0]
          [bads 0]
          [verbose (if (cdr verbose) (cdr verbose) #f)])
      (for-each
        (lambda (pr)
          (set! runs (+ runs 1))
      ...
      (if verbose
          (begin
            ...
            (display bads)
            (if (> runs 0)
                (begin (display "; Pass rate: ") (display (/ goods runs)))
                (display "; No tests."))
            (newline)))
  ...
  (define-s19-test!
    ...
    (lambda ()
      (not (null?
             (list (current-time 'time-tai) (current-time 'time-utc)
               (current-time 'time-monotonic) (current-time 'time-thread)
               (current-time 'time-process))))))
  (define-s19-test!
    "Testing time resolutions"
    (lambda ()
      (not (null?
             (list (time-resolution 'time-tai) (time-resolution 'time-utc)
  ...
  (define-s19-test!
    ...
    (lambda ()
      (and (tm:date=
             (time-tai->date (make-time time-tai 0 (+ 915148800 29)) 0)
             (make-date 0 58 59 23 31 12 1998 0))
           (tm:date=
             (time-tai->date (make-time time-tai 0 (+ 915148800 30)) 0)
             (make-date 0 59 59 23 31 12 1998 0))
           (tm:date=
             (time-tai->date (make-time time-tai 0 (+ 915148800 31)) 0)
             (make-date 0 60 59 23 31 12 1998 0))
           (tm:date=
             (time-tai->date (make-time time-tai 0 (+ 915148800 32)) 0)
             (make-date 0 0 0 0 1 1 1999 0)))))
  (define-s19-test!
    "Date-UTC Conversions"
    (lambda ()
      (and (time=?
             (make-time time-utc 0 (- 915148800 2))
             (date->time-utc (make-date 0 58 59 23 31 12 1998 0)))
           (time=?
             (make-time time-utc 0 (- 915148800 1))
             (date->time-utc (make-date 0 59 59 23 31 12 1998 0)))
           (time=?
             (make-time time-utc 0 (- 915148800 0))
...
(define (date->string/all-formats)
  (define fs
    '("~~" "~a" "~A" "~b" "~B" "~c" "~d" "~D" "~e" "~f" "~h" "~H"
      "~I" "~j" "~k" "~l" "~m" "~M" "~n" "~N" "~p" "~r" "~s" "~S"
      "~t" "~T" "~U" "~V" "~w" "~W" "~x" "~X" "~y" "~Y" "~z" "~Z"
      "~1" "~2" "~3" "~4" "~5"))
