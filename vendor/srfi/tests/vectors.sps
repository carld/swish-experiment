...
(or (equal?
      ...
      '#(a b c d e f g h i))
    (fail 'vector-copy:1))
(or (equal? (vector-copy '#(a b c d e f g h i) 6) '#(g h i))
    (fail 'vector-copy:2))
(or (equal?
...
(or (equal? (vector-append '#(x) '#(y)) '#(x y))
    (fail 'vector-append:1))
(or (equal? (vector-append '#(a) '#(b c d)) '#(a b c d))
...
(or (and (eq? (vector= eq? '#(a b c d) '#(a b c d)) #t)
         ...
         (eq? (vector= eq?) #t)
         (eq? (vector= eq? '#(a)) #t)
         (eq? (vector= eq? (vector (vector 'a)) (vector (vector 'a)))
              #f)
         (eq? (vector=
                equal?
                (vector (vector 'a))
              ...
              #t))
    (fail 'vector=))
(or (eq? (vector-ref '#(a b c d) 2) 'c) (fail 'vector-ref))
(or (eq? (vector-length '#(a b c)) 3) (fail 'vector-length))
(or (equal?
      (vector-fold
        (lambda (index len str) (max (string-length str) len))
        0
        '#("a" "b" "" "dd" "e"))
      2)
    (fail 'vector-fold:1))
(or (equal?
      (vector-fold
        (lambda (index tail elt) (cons elt tail))
        '()
        '#(0 1 2 3 4))
      '(4 3 2 1 0))
    (fail 'vector-fold:2))
...
(or (member
      (let ([count 0])
        (vector-map
          (lambda (ignored-index ignored-elt)
            (set! count (+ count 1))
            count)
          '#(a b)))
      '(#(1 2) #(2 1)))
...
(or (equal?
      (let ([p (open-output-string)])
        (vector-for-each
          (lambda (i x) (display x p) (newline p))
...
(or (equal?
      ...
      2)
    (fail 'vector-count:2))
(or (equal? (vector-index even? '#(3 1 4 1 5 9)) 2)
    (fail 'vector-index:1))
(or (equal?
...
(or (equal?
      ...
      #f)
    (fail 'vector-index:3))
(or (equal? (vector-index-right even? '#(3 1 4 1 5 9)) 2)
...
(or (equal? (vector-skip even? '#(3 1 4 1 5 9)) 0)
    (fail 'vector-skip:1))
(or (equal?
...
(or (equal?
      ...
      1)
    (fail 'vector-skip:4))
(or (equal? (vector-skip-right even? '#(3 1 4 1 5 9)) 5)
...
(define (string-comparator s1 s2)
  (cond
    [(< (string-length s1) (string-length s2)) -1]
    [(> (string-length s1) (string-length s2)) 1]
    [(string<? s1 s2) -1]
    [(string>? s1 s2) 1]
    [else 0]))
(or (equal?
      (vector-binary-search '#() "bad" string-comparator)
      #f)
    (fail 'vector-binary-search:0))
(or (equal?
      (vector-binary-search
        '#("ab" "cd" "ef" "bcd" "cde" "aaaa")
        "bad"
        string-comparator)
      #f)
...
(or (equal? (vector-any list '#() '#(a b c)) #f)
    (fail 'vector-any:0))
(or (equal? (vector-any list '#(a b c) '#()) #f)
    (fail 'vector-any:1))
(or (equal? (vector-any list '#(a b c) '#(d)) '(a d))
    (fail 'vector-any:2))
(or (equal?
      (vector-any memq '#(a b c) '#(() (c d e) (b c 97)))
      '(c 97))
    (fail 'vector-any:3))
(or (equal? (vector-every list '#() '#(a b c)) #t)
    (fail 'vector-every:0))
(or (equal? (vector-every list '#(a b c) '#()) #t)
    (fail 'vector-every:1))
(or (equal? (vector-every list '#(a b c) '#(d)) '(a d))
    (fail 'vector-every:2))
(or (equal?
    ...
    (fail 'vector-every:3))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-set! v 1 11) v)
      '#(0 11 2 3))
    (fail 'vector-set!))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-swap! v 1 3) v)
      '#(0 3 2 1))
    (fail 'vector-swap!))
(or (equal? (let ([v (vector)]) (vector-fill! v 97) v) '#())
    (fail 'vector-fill!:0))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-fill! v 97) v)
      '#(97 97 97 97))
    (fail 'vector-fill!:1))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-fill! v 97 1) v)
      '#(0 97 97 97))
    (fail 'vector-fill!:2))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-fill! v 97 1 2) v)
      '#(0 97 2 3))
    (fail 'vector-fill!:3))
(or (equal? (let ([v (vector)]) (vector-reverse! v) v) '#())
    (fail 'vector-reverse!:0))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-reverse! v) v)
      '#(3 2 1 0))
    (fail 'vector-reverse!:1))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-reverse! v 1) v)
      '#(0 3 2 1))
    (fail 'vector-reverse!:2))
(or (equal?
      (let ([v (vector 0 1 2 3)]) (vector-reverse! v 1 3) v)
      '#(0 2 1 3))
    (fail 'vector-reverse!:3))
(or (equal?
      (let ([v (vector)] [src '#(100 101 102 103 104 105)])
        (vector-copy! v 0 v)
        v)
      '#())
    (fail 'vector-copy!:0))
...
(or (equal? (vector->list '#()) '()) (fail 'vector->list:0))
(or (equal? (vector->list '#(a b c)) '(a b c))
    (fail 'vector->list:1))
(or (equal? (vector->list '#(a b c d e) 1) '(b c d e))
    (fail 'vector->list:2))
(or (equal? (vector->list '#(a b c d e) 1 4) '(b c d))
    (fail 'vector->list:3))
(or (equal? (reverse-vector->list '#()) '())
    (fail 'reverse-vector->list:0))
(or (equal? (reverse-vector->list '#(a b c)) '(c b a))
    (fail 'reverse-vector->list:1))
(or (equal?
      (reverse-vector->list '#(a b c d e) 1)
      '(e d c b))
    (fail 'reverse-vector->list:2))
(or (equal? (reverse-vector->list '#(a b c d e) 1 3) '(c b))
    (fail 'reverse-vector->list:3))
(or (equal? (list->vector '()) '#()) (fail 'list->vector:0))
(or (equal? (list->vector '(a b c)) '#(a b c))
    (fail 'list->vector:1))
(or (equal? (reverse-list->vector '()) '#())
    (fail 'reverse-list->vector:0))
(or (equal? (reverse-list->vector '(a b c)) '#(c b a))
