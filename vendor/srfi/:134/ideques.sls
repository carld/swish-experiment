...
(library (srfi :134 ideques)
  ...
  (import (except (rnrs) define-record-type remove fold-right)
    ...
    (only (srfi :1) fold-right unfold list= concatenate zip
      append-map)
    (srfi :9) (srfi :41) (srfi :158))
  (define (stream-count pred s)
    (stream-fold (lambda (n x) (if (pred x) (+ n 1) n)) 0 s))
  ...
  (define (make-deque lenf f lenr r)
    (cond
      [(> lenf (+ (* lenr stream-length-factor) 1))
       (let* ([i (quotient (+ lenf lenr) 2)]
              [j (- (+ lenf lenr) i)]
              [f. (stream-take i f)]
              [r. (stream-append r (stream-reverse (stream-drop i f)))])
         (%make-dq i f. j r.))]
      [(> lenr (+ (* lenf stream-length-factor) 1))
       (let* ([j (quotient (+ lenf lenr) 2)]
              [i (- (+ lenf lenr) j)]
              [r. (stream-take j r)]
              [f. (stream-append f (stream-reverse (stream-drop j r)))])
         (%make-dq i f. j r.))]
  ...
  (define (ideque . args)
    (if (null? args) *empty* (list->ideque args)))
  ...
  (define (%ideque=-binary elt= dq1 dq2)
    ...
    (or (eq? dq1 dq2)
        (and (%ideque-same-length dq1 dq2)
             (stream=?
               elt=
               (stream-append (dq-f dq1) (stream-reverse (dq-r dq1)))
               (stream-append (dq-f dq2) (stream-reverse (dq-r dq2)))))))
  ...
  (define (%ideque-take dq n)
    (let ([lenf (dq-lenf dq)] [f (dq-f dq)] [lenr (dq-lenr dq)])
      (if (<= n lenf)
    ...
    (cond
      ...
      [else
       (make-deque
         ...
         (stream-append (dq-f dq1) (stream-reverse (dq-r dq1)))
         (ideque-length dq2)
         (stream-append (dq-r dq2) (stream-reverse (dq-f dq2))))]))
  (define (ideque-count pred dq)
    (%check-ideque dq)
    (+ (stream-count pred (dq-f dq))
       (stream-count pred (dq-r dq))))
  ...
  (define (ideque-fold proc knil dq)
    (let ([proc* (lambda (acc x) (proc x acc))])
      (%check-ideque dq)
      (stream-fold
        proc*
  ...
  (define (ideque-partition pred dq)
    ...
    (receive
      (f1 f2)
      (stream-partition pred (dq-f dq))
      (receive
        (r1 r2)
        (stream-partition pred (dq-r dq))
        (values
  ...
  (define (ideque-take-while pred dq)
    ...
    (receive
      (hd tl)
      (stream-span pred (dq-f dq))
      (if (stream-null? tl)
          (receive
            (hd. tl.)
            (stream-span pred (stream-reverse (dq-r dq)))
            (make-deque
              (dq-lenf dq)
              (dq-f dq)
              (stream-length hd.)
  ...
  (define (%idq-span-break op pred dq)
    ...
    (receive
      ...
      (if (stream-null? tail)
          (receive
            (head. tail.)
            (op pred (stream-reverse (dq-r dq)))
            (values
              (make-deque
                (stream-length head)
                head
  ...
  (define (ideque-span pred dq)
    (%idq-span-break stream-span pred dq))
  (define (ideque-break pred dq)
    (%idq-span-break stream-break pred dq))
