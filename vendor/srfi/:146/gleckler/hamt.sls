...
;;;;;; File header: %3a146/gleckler/hamt.sls
;;;; Persistent Hash Map
(library (srfi :146 gleckler hamt)
  (export fragment->mask hamt->list hamt-fetch hamt-null
   hamt-null? hamt/count hamt/empty? hamt/for-each
   hamt/immutable hamt/mutable hamt/mutable? hamt/payload?
   hamt/put hamt/put! hamt/replace hamt/replace!
   hash-array-mapped-trie? make-hamt collision?
   hamt-bucket-size hamt-hash-size hamt/root leaf-stride
   narrow/array narrow/leaves narrow? next-set-bit wide/array
   wide/children wide?)
  (import (except (rnrs) define-record-type assert assoc)
    (only (chezscheme) remainder) (srfi :9)
    (only (srfi :1) find-tail assoc) (srfi :16)
    (only (srfi :143) fx-width) (srfi :151)
    (srfi :146 gleckler hamt-misc)
    (srfi :146 gleckler vector-edit))
  (define (error* msg . args) (error 'hamt msg args))
  (define hamt-hash-slice-size 5)
  (define hamt-hash-size
    (let ([word-size fx-width])
      (- word-size (remainder word-size hamt-hash-slice-size))))
  ...
  (define (hamt/empty? hamt) (zero? (hamt/count hamt)))
  (define (hamt/immutable-inner hamt replace)
    ...
    (if (hamt/mutable? hamt)
        (let ([payload? (hamt/payload? hamt)])
          (%make-hamt (hamt/= hamt) (hamt/count hamt) (hamt/hash hamt) #f
            payload? (->immutable (hamt/root hamt) payload? replace)))
        hamt))
  ...
  (define (hamt/mutable hamt)
    (if (hamt/mutable? hamt)
        hamt
        (%make-hamt (hamt/= hamt) (hamt/count hamt) (hamt/hash hamt)
          #t (hamt/payload? hamt) (hamt/root hamt))))
  ...
  (define (hamt/replace! hamt key dp)
    ...
    (let*-values ([(root) (hamt/root hamt)]
      ...
      (unless (eq? node root) (set-hamt/root! hamt node))
      hamt))
  (define (hamt/put! hamt key datum)
    (hamt/replace! hamt key (lambda (x) datum)))
  (define (make-empty-narrow) (make-narrow (vector) 0 0))
  (define (hamt-null? n) (eq? n hamt-null))
  (define (collision-single-leaf? n)
    (let ([elements (collision/entries n)])
      (and (not (null? elements)) (null? (cdr elements)))))
    ...
    (cond
      ...
      [(wide? n)
       (let* ([c (wide/children n)]
              ...
              [l-count (bit-count l)]
              [a-in (wide/array n)]
              [a-out (make-vector (+ (* stride l-count) (bit-count c)))])
         (let next-leaf ([start 0] [count 0])
           (let ([i (next-set-bit l start hamt-bucket-size)])
             (when i
               (let* ([j (* stride i)] [key (vector-ref a-in j)])
                 (vector-set! a-out count key)
                 (when payload?
                   (vector-set!
                     a-out
                     (+ count 1)
                     (replace key (vector-ref a-in (+ j 1))))))
               (next-leaf (+ i 1) (+ stride count)))))
  ...
  (define (fragment->mask fragment) (- (expt 2 fragment) 1))
  (define (mutate hamt n shift dp h k)
    (cond
      [(collision? n) (modify-collision hamt n shift dp h k)]
      [(narrow? n)
       (modify-wide hamt (narrow->wide n (hamt/payload? hamt))
         shift dp h k)]
  ...
  (define (modify-wide hamt n shift dp h k)
    (let ([fragment (hash-fragment shift h)])
      (cond
        [(bit-set? fragment (wide/children n))
  ...
  (define (modify-wide-child hamt n shift dp h k)
    (let*-values ([(fragment) (hash-fragment shift h)]
                  ...
                  [(change new-child)
                   (mutate hamt child (+ shift hamt-hash-slice-size) dp h
                     k)])
      (define (coalesce key datum)
        (vector-set! array i key)
        (when payload? (vector-set! array (+ i 1) datum))
        (set-wide/children!
          n
          (copy-bit fragment (wide/children n) #f))
  ...
  (define (add-wide-leaf-key hamt n shift d h k)
    ...
    (define make-entry (if payload? cons (lambda (k d) k)))
    (let* ([fragment (hash-fragment shift h)]
           [array (wide/array n)]
           [stride (leaf-stride payload?)]
           [i (* stride fragment)]
      ...
      (vector-set!
        ...
        (if (= h hash)
            (make-collision
              (list (make-entry k d) (make-entry key datum))
              h)
            (make-narrow-with-two-keys payload?
              (+ shift hamt-hash-slice-size) h k d hash key datum)))
      (when payload? (vector-set! array (+ i 1) #f))
      (set-wide/children!
        n
        (copy-bit fragment (wide/children n) #t))
      (set-wide/leaves! n (copy-bit fragment (wide/leaves n) #f))
      (values 1 n)))
  (define (modify-wide-new hamt n shift d h k)
    (let* ([fragment (hash-fragment shift h)]
      ...
      (set-wide/leaves! n (copy-bit fragment (wide/leaves n) #t))
      (values 1 n)))
  (define (make-narrow-with-two-keys payload? shift h1 k1 d1
           h2 k2 d2)
    (define (two-leaves f1 k1 d1 f2 k2 d2)
      (make-narrow
        (if payload? (vector k1 d1 k2 d2) (vector k1 k2))
        0
      ...
      (cond
        [(= f1 f2)
         (make-narrow
           (vector
             (make-narrow-with-two-keys payload?
               (+ shift hamt-hash-slice-size) h1 k1 d1 h2 k2 d2))
           (copy-bit f1 0 #t)
           0)]
        [(< f1 f2) (two-leaves f1 k1 d1 f2 k2 d2)]
        [else (two-leaves f2 k2 d2 f1 k1 d1)])))
  (define (modify-pure hamt n shift dp h k)
    (cond
      ...
      [(wide? n)
       (error*
         "Should have been converted to narrow before here.")]
      [else (error* "Unknown HAMT node type." n)]))
      ...
      (if (hamt-null? d)
          ...
          (values
            ...
            (let descend ([shift shift])
              (let ([collision-fragment (hash-fragment
                                          shift
                                          collision-hash)]
  ...
  (define (leaf-stride payload?) (if payload? 2 1))
  ...
  (define (modify-narrow hamt n shift dp h k)
    (let ([fragment (hash-fragment shift h)])
      (cond
        [(bit-set? fragment (narrow/children n))
  ...
  (define (modify-narrow-child hamt n shift dp h k)
    (let*-values ([(fragment) (hash-fragment shift h)]
                  ...
                  [(change new-child)
                   (modify-pure hamt child (+ shift hamt-hash-slice-size)
                     dp h k)])
      (define (coalesce key datum)
      ...
      (define (replace)
        (values
          change
          (make-narrow
            (vector-replace-one array child-index new-child)
            c
            l)))
      (cond
        [(eq? new-child child) (values 0 n)]
        [(hamt-null? new-child)
         (error* "Child cannot become null." n)]
        [(collision? new-child)
         (if (collision-single-leaf? new-child)
        ...
        [(narrow? new-child)
         (if (narrow-single-leaf? new-child)
             (let ([a (narrow/array new-child)])
               (coalesce (vector-ref a 0) (and payload? (vector-ref a 1))))
             (replace))]
        [(wide? new-child)
            ...
            (cond
              [(hamt-null? d)
               (values
                 -1
                 (make-narrow
                   (vector-without array leaf-index (+ leaf-index stride))
                   c
              ...
              [payload?
               (values
                 0
                 (make-narrow
                   (vector-replace-one array (+ leaf-index 1) d)
                   c
                   l))]
              [else (values 0 n)]))
          (let ([d (dp hamt-null)])
        ...
        (make-narrow
          (if (= h hash)
              ...
              (vector-edit
                ...
                (add child-index
                     (make-narrow-with-two-keys payload?
                       (+ shift hamt-hash-slice-size) h k d hash key
                       datum))))
  ...
  (define (modify-narrow-new hamt n shift d h k)
    (let* ([fragment (hash-fragment shift h)]
      ...
      (values
        ...
        (make-narrow
          (if payload?
              (vector-edit array (add leaf-index k) (add leaf-index d))
              (vector-edit array (add leaf-index k)))
          c
          (copy-bit fragment l #t)))))
  (define (hamt-fetch hamt key)
    ...
    (let ([h (hash-bits (hamt/hash hamt) key)]
          [payload? (hamt/payload? hamt)])
      (let descend ([n (hamt/root hamt)] [shift 0])
        (cond
          [(collision? n)
           (let ([entries (collision/entries n)] [key= (hamt/= hamt)])
             (if payload?
                 (cond [(assoc key entries key=) => cdr] [else hamt-null])
               ...
               [(bit-set? fragment c)
                (let* ([mask (fragment->mask fragment)]
                       [child-index (narrow-child-index
                                      ...
                                      (hamt/payload? hamt))])
                  (descend
                    (vector-ref array child-index)
                    (+ shift hamt-hash-slice-size)))]
               [(bit-set? fragment l)
                (let* ([mask (fragment->mask fragment)]
                       [leaf-index (narrow-leaf-index
                                     l
                                     mask
                                     (hamt/payload? hamt))]
          ...
          [(wide? n)
           (let ([array (wide/array n)]
             ...
             (cond
               [(bit-set? i c)
                (descend
                  (vector-ref array (* stride i))
                  (+ shift hamt-hash-slice-size))]
               [(bit-set? i l)
                (let* ([j (* stride i)] [k (vector-ref array j)])
                  (if ((hamt/= hamt) k key)
                      (if payload? (vector-ref array (+ j 1)) 'present)
                      hamt-null))]
               [else hamt-null]))]
          [else (error* "Unexpected type of child node.")]))))
      ...
      (let next-leaf ([count 0] [start 0])
        (let ([i (next-set-bit l start hamt-bucket-size)])
          (if i
              ...
              (let next-child ([start 0] [offset (* stride count)])
                (let ([i (next-set-bit c start hamt-bucket-size)])
                  (when i
  ...
  (define (hamt-node/for-each node payload? procedure)
    (cond
      [(collision? node)
       (collision/for-each procedure node payload?)]
  ...
  (define (hamt->list hamt procedure)
    (let ([accumulator '()])
      (hamt/for-each
        (lambda (k v)
          (set! accumulator (cons (procedure k v) accumulator)))
        hamt)
      accumulator))
  (define (assert-collision-valid node hp payload?)
    ...
    (let ([entries (collision/entries node)]
      ...
      (if payload? (map car entries) entries)))
  (define (assert-narrow-valid node hp payload? shift)
    ...
    (let ([array (narrow/array node)]
      ...
      (let next-leaf ([count 0] [i 0] [keys '()])
  ...
  (define (assert-wide-valid node hp payload? shift)
    ...
    (let ([array (wide/array node)]
          ...
          [l (wide/leaves node)])
      (assert (zero? (bitwise-and c l)))
      (let next-fragment ([i 0] [key-groups '()])
        (if (= i hamt-bucket-size)
            (apply append key-groups)
            (let ([j (* stride i)])
              (cond
                ...
                [(bit-set? i c)
                 (let* ([child (vector-ref array j)]
                   ...
                   (do-list
                     (k child-keys)
                     (assert (= i (hash-fragment shift (hash-bits hp k)))))
                   (next-fragment (+ i 1) (cons child-keys key-groups)))]
                [else
                 (assert (not (vector-ref array j)))
                 (when payload? (assert (not (vector-ref array (+ j 1)))))
  ...
  (define (assert-hamt-node-valid node hp payload? shift)
    ...
    (cond
      [(collision? node)
       (assert-collision-valid node hp payload?)]
      [(narrow? node)
       (assert-narrow-valid node hp payload? shift)]
