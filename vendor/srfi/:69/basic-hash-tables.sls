...
;;;;;; File header: %3a69/basic-hash-tables.sls
#!r6rs
...
(library (srfi :69 basic-hash-tables)
  (export make-hash-table hash-table? alist->hash-table
   ...
   hash-table-values hash-table-walk hash-table-fold
   hash-table->alist hash-table-copy hash-table-merge! hash
  ...
  (define make-hash-table
    (case-lambda
      ...
      [(eql?)
       (cond
         [(eq? eql? eq?) (make-eq-hashtable)]
         [(eq? eql? eqv?) (make-eqv-hashtable)]
         [(eq? eql? equal?) (make-hashtable equal-hash eql?)]
         [(eq? eql? string=?) (make-hashtable rnrs:string-hash eql?)]
         [(eq? eql? string-ci=?)
          (make-hashtable rnrs:string-ci-hash eql?)]
         [else
          (assertion-violation 'make-hash-table
            "unrecognized equivalence predicate"
            eql?)])]
      ...
      (for-each
        (lambda (entry)
          (hashtable-update!
            table
            (car entry)
            (lambda (x) (if (eq? x not-there) (cdr entry) x))
            not-there))
        alist)
      table))
    ...
    (case-lambda
      [(table key thunk)
       (let ([val (hashtable-ref table key not-there)])
         (if (eq? val not-there) (thunk) val))]
      [(table key)
       (hash-table-ref
         table
         key
         (failure-thunk 'hash-table-ref key))]))
    ...
    (case-lambda
      [(table key proc thunk)
       (hashtable-update!
         table
         key
         (lambda (val) (if (eq? val not-there) (thunk) (proc val)))
         not-there)]
      [(table key proc)
       (hash-table-update!
         table
  ...
  (define (hash-table-merge! table1 table2)
    (hash-table-walk
      table2
      (lambda (k v) (hashtable-set! table1 k v)))
    table1)
  (define (make-hasher hash-proc)
    (case-lambda
      [(obj) (mod (hash-proc obj) (greatest-fixnum))]
      [(obj bound) (mod (hash-proc obj) bound)]))
  (define hash (make-hasher equal-hash))
  (define hash-by-identity (make-hasher equal-hash))
